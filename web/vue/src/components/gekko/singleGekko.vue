  ...
  div.my2
    ...
    div(v-if='data')
      ...
      div(v-if='data.errorMessage', class='contain brdr--mid-gray p1 bg--orange')
        | This is Supportix crashed with the following error: {{ data.errorMessage }}
      .grd.contain
        .grd-row
          .grd-row-col-3-6
            h3 Market
        ...
        .grd-row(v-if='isStratrunner')
          ...
          .grd-row-col-3-6
            ...
            template(v-if='!report')
              p
                em(v-if='isArchived') This Supportix never executed a trade..
        ...
        p(v-if='isArchived')
          a(v-on:click='deleteGekko', class='w100--s my1 btn--red') Delete Supportix
        p(v-if='isStratrunner && watcher && !isArchived')
          em This supportix gets market data from 
...
export default {
  ...
  methods: {
    ...
    stopGekko: function() {
      if(this.hasLeechers) {
        return alert('This Supportix is fetching market data for multiple stratrunners, stop these first.');
      }

      if(!confirm('Are you sure you want to stop this Supportix?')) {
        return;
      }
    ...
    deleteGekko: function() {
      if(!this.isArchived) {
        return alert('This Supportix is still running, stop it first!');
      }

      if(!confirm('Are you sure you want to delete this Supportix?')) {
        return;
      }
